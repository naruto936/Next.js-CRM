...
type ServiceCompletionsTableProps = {
  filtersOpen?: boolean;
  onOpenFilters?: () => void;
...
function RecordCard({
  ...
  row: ServiceCompletionRecord;
  columns: { apiName: string; label: string; dataType: string }[];
}) {
  const title =
    row.fields.Name?.trim() ||
    row.fields.Location_Name?.trim() ||
    `Service completion ${row.id}`;
...
export default function ServiceCompletionsTable({
  filtersOpen = false,
  onOpenFilters,
  searchCriteria = null,
  customViewId = null,
...
}: ServiceCompletionsTableProps) {
  const [records, setRecords] = useState<ServiceCompletionRecord[]>([]);
  ...
  const columnMeta = useMemo(
    () =>
      SERVICE_COMPLETION_LIST_FIELDS.map((apiName) => ({
        apiName,
  ...
  const fieldsParam = useMemo(
    () => encodeURIComponent(SERVICE_COMPLETION_LIST_FIELDS.join(",")),
    [],
  );
    ...
    async function loadRecords() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/service-completions?page=${page}&fields=${fieldsParam}${criteriaPart}${cvidPart}`,
        );
        const data = (await res.json()) as {
          records?: ServiceCompletionRecord[];
          totalCount?: number;
          hasMore?: boolean;
          filtered?: boolean;
          error?: string;
        ...
        if (!res.ok) {
          throw new Error(data.error ?? "Failed to load service completions");
        }
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Failed to load service completions");
          setRecords([]);
          setTotalCount(null);
          setHasMore(false);
        }
      ...
      <div className="relative z-20 shrink-0 border-b border-crm-border bg-crm-panel px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
          ...
          <div className="flex min-w-0 flex flex-1 flex-wrap items-baseline gap-x-2 gap-y-1 sm:gap-x-3">
            <h1 className="page-heading truncate text-base sm:text-lg">Service completions</h1>
