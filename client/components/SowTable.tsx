...
function StagePill({ stage }: { stage: string }) {
  ...
  const tone =
    ...
    : lower.includes("lost") || lower.includes("closed") ?
...
function getColumnWidthPx(col: { apiName: string; dataType?: string }) {
  if (col.apiName === "Stage") return 140;
...
function RecordCard({
  ...
  const title =
    row.fields.Deal_Name?.trim() ||
    row.fields.SOWID?.trim() ||
    `SOW ${row.id}`;
...
export default function SowTable({
  ...
  const fieldsParam = useMemo(
    () => encodeURIComponent(SOW_LIST_FIELDS.join(",")),
    [],
  );
    ...
    async function loadRecords() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/sow?page=${page}&fields=${fieldsParam}${criteriaPart}${cvidPart}`,
        );
        ...
        if (!res.ok) {
          throw new Error(data.error ?? "Failed to load SOW records");
        }
      ...
      <div className="relative z-20 shrink-0 border-b border-crm-border bg-crm-panel px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
          ...
          <div className="flex min-w-0 flex flex-1 flex-wrap items-baseline gap-x-2 gap-y-1 sm:gap-x-3">
            <h1 className="page-heading truncate text-base sm:text-lg">SOW</h1>
