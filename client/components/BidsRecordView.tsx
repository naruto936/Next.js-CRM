...
import { BIDS_DETAIL_SECTIONS, labelForBidsField } from "@/lib/bidsConfig";
import type { BidLineItemRow } from "@/lib/bidsStaticDetail";
...
function StatusPill({ status }: { status: string }) {
  ...
  const tone =
    ...
    : lower === "declined" ? "bg-zinc-100 text-zinc-800 ring-zinc-500/20 dark:bg-zinc-700/50 dark:text-zinc-200"
...
function BidLineItemsSection({ rows }: { rows: BidLineItemRow[] }) {
  if (rows.length === 0) {
    return <p className="text-sm text-crm-text-muted">No line items on this bid.</p>;
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-crm-table-head hover:bg-crm-table-head">
            ...
            <TableHead className="text-right text-crm-text">Unit price</TableHead>
            <TableHead className="text-right text-crm-text">Client price</TableHead>
            <TableHead className="text-right text-crm-text">Line total</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {rows.map((row) => (
...
type BidsRecordViewProps = {
  id: string;
};

export default function BidsRecordView({ id }: BidsRecordViewProps) {
  const [record, setRecord] = useState<BidRecord | null>(null);
  const [lineItems, setLineItems] = useState<BidLineItemRow[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/bids/${encodeURIComponent(id)}`, { cache: "no-store" });
        const data = (await res.json()) as {
          record?: BidRecord;
          lineItems?: BidLineItemRow[];
          layoutLabel?: string;
          zohoRecordId?: string;
          error?: string;
        };
  ...
  const title = useMemo(() => {
    ...
    return record.fields.Name?.trim() || record.fields.Bid_Number?.trim() || `Bid ${record.id}`;
  }, [record]);
  ...
  const sectionGroups = useMemo(() => {
    if (!record) return [];
    return BIDS_DETAIL_SECTIONS.map((section) => {
      ...
      const rows: RecordFieldRow[] = section.fields.map((apiName) => ({
        ...
        dataType:
          ...
          : apiName === "Is_Awarded" ? "boolean"
          : apiName === "Amount" ? "currency"
          : "text",
      }));
          ...
          <div className="space-y-4">
            ...
            <section className="overflow-hidden rounded-lg border border-crm-border bg-crm-panel">
              <div className="border-b border-crm-border px-4 py-3">
                <h2 className="section-heading text-sm sm:text-base">Bid line items</h2>
                <p className="mt-0.5 text-xs text-crm-text-muted">Services and pricing on this bid</p>
              </div>
              <div className="px-4 py-4">
                <BidLineItemsSection rows={lineItems} />
              </div>
            </section>
          </div>
        : !error ?
