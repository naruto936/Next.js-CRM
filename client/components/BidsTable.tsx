...
export default function BidsTable({
  ...
  const staticFilteredRecords = useMemo(() => {
    if (!USE_STATIC_LIST_DATA) return [] as BidRecord[];
    return filterStaticBidRecords(BIDS_STATIC_RECORDS, {
      fieldSelections,
      customViewId,
    });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const filtered =
        ...
        (customViewId != null && customViewId !== BIDS_STATIC_ALL_VIEW_ID);
