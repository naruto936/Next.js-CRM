...
import { labelForVendorField, VENDOR_LIST_FIELDS } from "@/lib/vendorConfig";
import type { ContractFieldFilterSelection } from "@/lib/contractFilterTypes";
import {
  filterStaticVendorRecords,
  VENDOR_STATIC_ALL_VIEW_ID,
  VENDOR_STATIC_RECORDS,
...
function openVendorRecord(recordId: string) {
  window.open(`/vendors/${recordId}`, "_blank", "noopener,noreferrer");
}
...
function isLongTextColumn(apiName: string) {
  return apiName === "Name" || apiName === "Email" || apiName === "Owner";
}
...
function getColumnWidthPx(col: { apiName: string; dataType?: string }) {
  if (col.apiName === "Vendor_Status") return 128;
...
function RecordCard({
  ...
  const title = row.fields.Name?.trim() || `Vendor ${row.id}`;
  ...
  return (
    <article
      ...
      tabIndex={0}
      title={title}
      onClick={() => openVendorRecord(row.id)}
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          openVendorRecord(row.id);
        }
      }}
...
export default function VendorTable({
  ...
  const columnMeta = useMemo(
    () =>
      VENDOR_LIST_FIELDS.map((apiName) => ({
        apiName,
  ...
  const fieldsParam = useMemo(
    () => encodeURIComponent(VENDOR_LIST_FIELDS.join(",")),
    [],
  );
  ...
  const staticFilteredRecords = useMemo(() => {
    if (!USE_STATIC_LIST_DATA) return [] as VendorRecord[];
    return filterStaticVendorRecords(VENDOR_STATIC_RECORDS, {
      fieldSelections,
      customViewId,
    });
    ...
    async function loadRecords() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/vendors?page=${page}&fields=${fieldsParam}${criteriaPart}${cvidPart}`,
        );
      ...
      <div className="relative z-20 shrink-0 border-b border-crm-border bg-crm-panel px-3 py-3 sm:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
          ...
          <div className="flex min-w-0 flex flex-1 flex-wrap items-baseline gap-x-2 gap-y-1 sm:gap-x-3">
            <h1 className="page-heading truncate text-base sm:text-lg">Vendors</h1>
